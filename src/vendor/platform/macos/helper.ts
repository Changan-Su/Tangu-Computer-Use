...
export const HELPER_BUNDLE_ID = "com.forsion.tangu-computer-use";
...
const DEFAULT_HELPER_SOCKET_PATH = path.join(os.homedir(), "Library", "Caches", "tangu-computer-use", "bridge.sock");
...
export class MacosHelperClient {
	...
	async ensureInstalled(signal?: AbortSignal): Promise<void> {
		...
		if (!(await isExecutable(HELPER_APP_EXECUTABLE_PATH))) {
			throw new Error(`Failed to install tangu-computer-use helper app at ${HELPER_APP_PATH}.`);
		}
	}
	...
	async command<T>(cmd: string, args: Record<string, unknown> = {}, options?: { timeoutMs?: number; signal?: AbortSignal }): Promise<T> {
		...
		if (!(await this.ensureDaemon(options?.signal))) {
			throw new HelperTransportError(`tangu-computer-use helper app daemon is unavailable at ${HELPER_APP_PATH}.`);
		}
		try {
	...
	async restart(signal?: AbortSignal): Promise<void> {
		...
		if (!(await this.ensureDaemon(signal))) {
			throw new Error(`tangu-computer-use helper did not come back after restart. Helper app: ${HELPER_APP_PATH}`);
		}
	}
	...
	async ensureProtocol(signal?: AbortSignal): Promise<PlatformDiagnostics> {
		...
		if (diagnostics.protocolVersion !== HELPER_PROTOCOL_VERSION || !relaunchedExecutableMatches) {
			this.daemonAvailable = false;
			throw new Error(
				`tangu-computer-use helper mismatch after relaunch: expected protocol ${HELPER_PROTOCOL_VERSION} and executable ${HELPER_APP_EXECUTABLE_PATH}; got protocol ${diagnostics.protocolVersion} and executable ${diagnostics.executablePath ?? "unknown"}. Reinstall or rebuild the helper app at ${HELPER_APP_PATH}.`,
			);
		}
		return diagnostics;
	}
