...
const GRANT_INSTRUCTIONS =
	"Grant Accessibility and Screen Recording to tangu-computer-use.app in System Settings → Privacy & Security. " +
...
const SIGNING_MIGRATION_WARNING =
	...
	"tangu-computer-use.app was re-signed. Re-enable both toggles for the newly signed helper. " +
...
function permissionPrompt(status: PermissionStatus, helperPath: string, hint?: string): string {
	return [
		"tangu-computer-use needs macOS permissions for its helper app.",
		permissionStatusSummary(status),
		"",
		`Helper: tangu-computer-use.app (${helperPath})`,
		hint,
		"",
		`Important: ${SIGNING_MIGRATION_WARNING}`,
		"",
		"tangu-computer-use.app is already listed in the pane(s) — enable its toggle, then choose Recheck.",
	].filter(Boolean).join("\n");
}
...
async function registerPermissions(signal?: AbortSignal): Promise<void> {
	...
	// capture attempt so tangu-computer-use.app is pre-listed in both Settings
...
export async function ensureMacosReady(
	...
	if (!(await macosHelper.ensureDaemon(signal))) {
		throw new Error(`tangu-computer-use helper app daemon did not start. Helper app: ${HELPER_APP_PATH}`);
	}
	...
	if (!permissionStatus.accessibility || !permissionStatus.screenRecording) {
		...
		const attributionHint = permissionStatus.source?.attribution === "caller"
			? `Warning: the helper is not running as the installed tangu-computer-use.app (executable: ${permissionStatus.source?.executablePath ?? "unknown"}). Grants made now would attach to the launching app instead. Restart Tangu so the canonical helper is used.`
			: undefined;
		permissionStatus = await ensurePermissions(
			ctx,
			{
				...
				copy: {
					nonInteractiveError: (helperPath) => `tangu-computer-use setup requires an interactive session. Run 'tangu computer-use setup' in a terminal. ${GRANT_INSTRUCTIONS}\nHelper path: ${helperPath}`,
					prompt: permissionPrompt,
					incompleteError: (helperPath) => `tangu-computer-use setup is incomplete. ${GRANT_INSTRUCTIONS} Helper path: ${helperPath}`,
					readyMessage: "tangu-computer-use is ready.",
