...
export const WINDOWS_HELPER_PATH = process.env.PI_COMPUTER_USE_WINDOWS_HELPER_PATH || path.join(os.homedir(), ".pi", "agent", "helpers", "tangu-computer-use", "windows-bridge.exe");
...
export class WindowsHelperClient {
	...
	dispose(): void {
		const error = new Error("Windows helper closed because the Tangu session ended.");
	...
	private onStdout(chunk: string): void {
		...
		for (;;) {
			...
			if (parsed.protocolVersion !== WINDOWS_HELPER_PROTOCOL_VERSION) {
				pending.reject(new Error(`Windows helper protocol mismatch: expected ${WINDOWS_HELPER_PROTOCOL_VERSION}, got ${parsed.protocolVersion ?? "unknown"}. Restart Tangu to use the installed helper.`));
