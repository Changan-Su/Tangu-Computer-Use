...
export const LINUX_HELPER_PATH = process.env.PI_COMPUTER_USE_LINUX_HELPER_PATH
	|| path.join(os.homedir(), ".pi", "agent", "helpers", "tangu-computer-use", "linux-bridge");
...
export class LinuxHelperClient {
	...
	dispose(): void {
		this.rejectPending(new Error("Linux helper closed because the Tangu session ended."));
	...
	private onStdout(chunk: string): void {
		...
		for (;;) {
			...
			if (parsed.protocolVersion !== LINUX_HELPER_PROTOCOL_VERSION) {
				pending.reject(new Error(`Linux helper protocol mismatch: expected ${LINUX_HELPER_PROTOCOL_VERSION}, got ${parsed.protocolVersion ?? "unknown"}. Restart Tangu to use the installed helper.`));
