...
final class InputSuppressionGuard {
	...
	func begin() throws {
		...
		thread.name = "tangu-computer-use-input-suppression"
		lock.lock()
		tapThread = thread
		lock.unlock()
		thread.start()
...
final class Bridge {
	...
	// Tangu 新增:最近一次被操控的窗口(供边缘光效跟随 + liveView 取画面)。serve 线程写、主线程读。
	private let controlledLock = NSLock()
	private var controlledWindowId: UInt32 = 0
	private var controlledPid: Int32 = 0
	private var controlledAt: Date?
	private var highlightFollowTimer: Timer?
	...
	private func defaultSocketPath() -> String {
		...
		return "\(home)/Library/Caches/tangu-computer-use/bridge.sock"
	}
	...
	// MARK: - Tangu 扩展:被操控窗口的边缘光效 + 实时画面(非上游)

	/// 记下"现在正被操控的是哪个窗口",并按需点亮边缘光效。每次 act 调一次。
	/// 记录与光效分开:即使用户关了光效,liveView 仍要知道该看哪个窗口。
	private func noteControlledWindow(windowId: UInt32, pid: Int32, highlight: Bool) {
		controlledLock.lock()
		controlledWindowId = windowId
		controlledPid = pid
		controlledAt = Date()
		controlledLock.unlock()
		guard supportsAgentCursor, highlight else { return }
		Task { @MainActor in AgentHighlight.shared.show(windowId: windowId) }
	}

	private func controlledSnapshot() -> (windowId: UInt32, pid: Int32, at: Date)? {
		controlledLock.lock()
		defer { controlledLock.unlock() }
		guard controlledWindowId != 0, let at = controlledAt else { return nil }
		return (controlledWindowId, controlledPid, at)
	}

	/// 最近被操控窗口的一帧画面。给 Forsion 桌面的"被操控窗口"视图轮询用(≤2fps)。
	/// 只读:**绝不**启动 helper、绝不改焦点。没有正在被操控的窗口就 active=false。
	/// 拿不到画面(如未授予录屏)也照实回 active=true + error,不编一张图。
	private func liveView(_ request: [String: Any]) throws -> [String: Any] {
		let activeWithinMs = optionalIntArg(request, "activeWithinMs") ?? 120_000
		guard let controlled = controlledSnapshot() else { return ["active": false] }
		let ageMs = Int(Date().timeIntervalSince(controlled.at) * 1000)
		if ageMs > max(0, activeWithinMs) { return ["active": false] }

		var payload: [String: Any] = ["active": true, "windowId": Int(controlled.windowId), "pid": Int(controlled.pid), "ageMs": ageMs]
		if let app = NSRunningApplication(processIdentifier: controlled.pid) {
			if let name = app.localizedName { payload["app"] = name }
			if let bundleId = app.bundleIdentifier { payload["bundleId"] = bundleId }
		}
		if let window = windowElement(pid: controlled.pid, windowId: controlled.windowId),
			let title = stringAttribute(window, attribute: kAXTitleAttribute as CFString) {
			payload["title"] = title
		}
		guard request["image"] as? Bool ?? true else { return payload }
		let maxDimension = optionalIntArg(request, "maxDimension") ?? 1280
		let quality = (request["quality"] as? NSNumber)?.doubleValue ?? 0.6
		// 先问常驻取景流要最新一帧(真·实时;见 live_stream.swift)。流还没起来 —— 首帧、刚换窗口 ——
		// 退到**同样按窗口尺寸出图**的单帧兜底,所以第一帧照样立刻有画面、也照样没有留白。
		// `source` 让调用方看得见走的哪条路。
		if #available(macOS 14.0, *) {
			let live = LiveWindowStream.shared
			var source = "stream"
			var image: CGImage?
			if let fresh = live.latestFrame(windowId: controlled.windowId, maxDimension: maxDimension) {
				payload["frameSeq"] = String(fresh.seq)
				// 静止的窗口不会有新帧。调用方带着上次见过的帧号来,一样就直接说"没变" ——
				// 否则同一张图会被每秒重编 8 次 JPEG + base64 + 过 socket + 换一次 <img src>,纯白烧。
				if let seen = optionalStringArg(request, "sinceFrame"), seen == String(fresh.seq) {
				...
				source = "snapshot"
				image = live.captureOnce(windowId: controlled.windowId, maxDimension: maxDimension)
			}
			if let image {
				guard let jpeg = jpegData(image: image, quality: quality) else {
					payload["error"] = "encode_failed"
					return payload
				}
				payload["width"] = image.width
				payload["height"] = image.height
				payload["jpegBase64"] = jpeg.base64EncodedString()
				payload["source"] = source
				return payload
			}
		}
		// 最后一条老路(macOS <14,或上面两条都拿不到):上游的 captureWindow。它按整块屏幕出图,
		// 窗口缩在左上角、四周留白 —— 有画面总比没有强,但打上 legacy 标记别让人以为这是正常尺寸。
		do {
			let captured = try captureWindow(windowId: controlled.windowId)
			let image = downscaledImage(captured.image, maxDimension: maxDimension) ?? captured.image
			guard let jpeg = jpegData(image: image, quality: quality) else {
				payload["error"] = "encode_failed"
				return payload
			}
	...
	private func permissionSource() -> [String: Any] {
		...
		if executable.contains("/tangu-computer-use.app/Contents/MacOS/"), parentPid == 1 {
	...
	private func storeLookRecord(_ record: LookRecord) {
		...
		// Tangu:每次观察也算"agent 正在看这个窗口",liveView 据此选画面(唯一的 look 收口点,
		// 比在 look() 的各个分支里各挂一次可靠)。**不点光效** —— 光效只表示"正在动手"。
		if record.windowId != 0 {
			controlledLock.lock()
			controlledWindowId = record.windowId
			controlledPid = pidForWindowId(record.windowId) ?? controlledPid
			controlledAt = Date()
			controlledLock.unlock()
		}
	}
	...
	private func act(_ request: [String: Any]) throws -> [String: Any] {
		...
		// Tangu:登记"正在操控这个窗口"(供 liveView),并按设置点亮边缘光效。放在最前面 —— 后面任何
		// 一步抛错都不该让用户失去"agent 正在动这个窗口"的可见信号。
		// 复用 cursorOverlay 这一个开关(vendor 的 act 请求里本就带着它),不为光效另开配置键:
		// 二者是同一件事——"把 agent 的动作画出来给人看"。要拆开时再加键不迟。
		noteControlledWindow(windowId: record.windowId, pid: pid, highlight: request["cursorOverlay"] as? Bool ?? true)
		...
		/// `viaAx = true`:这一下是用 AX 直接按压完成的,压根没发物理事件 —— 无论 delivery 名义上
		/// 是什么,系统指针都没动过,所以虚拟光标该画。否则前台策略下用 AX 打成的那一下毫无反馈。
		func animateCursor(at point: CGPoint, viaAx: Bool = false) {
			guard supportsAgentCursor,
				(request["cursorOverlay"] as? Bool ?? true),
				viaAx || (delivery == "pid" && policy != "ax_only"),
		...
		/// Tangu 新增:**纯坐标点击先做一次 AX 命中测试**。命中可按压元素就直接 AXPress ——
		/// 不抢前台、不动真鼠标、画虚拟光标。命不中再照上游回落物理事件。
		///
		/// 为什么需要它:物理事件那条后台路(CGEventPostToPid)对非 key 窗口的 App 会被直接丢掉,
		/// 所以上游对坐标点击一律直接判 `needsForeground`(actions.ts)走 HID 前台。于是"看着截图
		/// 盲点"必然抢前台 —— 这是用户实报的症结。AX 按压是唯一能在后台真正落下去的点击。
		///
		/// ⚠️命中测试的根**必须是目标 App**,不能用 systemWide:目标窗口在后台时,那个屏幕坐标上压着的
		/// 是别人的窗口,systemWide 会把别人窗口里的控件返回给我们 —— 那一下就点到别的 App 上了。
		/// 再加一道窗口子树校验:同一个 App 开多个窗口时,同样会点错窗口。
		func backgroundPressAtPoint(_ point: CGPoint) -> Bool {
			guard action == "press" || action == "click" else { return false }
			// 右键/中键/双击的语义 AXPress 表达不了,老实走物理事件。
			guard (params["button"] as? String ?? "left") == "left",
				((params["clickCount"] as? NSNumber)?.intValue ?? 1) <= 1
			else { return false }
			var hitRef: AXUIElement?
			guard AXUIElementCopyElementAtPosition(AXUIElementCreateApplication(pid), Float(point.x), Float(point.y), &hitRef) == .success,
				let hit = hitRef
			else { return false }
			var hitPid: pid_t = 0
			guard AXUIElementGetPid(hit, &hitPid) == .success, hitPid == pid else { return false }
			guard let windowRoot = windowElement(pid: pid, windowId: record.windowId),
				sameElement(hit, windowRoot) || isElement(hit, descendantOf: windowRoot)
			else { return false }
			// 与 ref 路径同一条规则:网页内容与文本框要的是真实指针焦点,AXPress 打不动。
			let hitRole = stringAttribute(hit, attribute: kAXRoleAttribute as CFString) ?? ""
			let textRoles: Set<String> = ["AXTextField", "AXTextArea", "AXTextView", "AXSearchField", "AXComboBox", "AXEditableText", "AXSecureTextField"]
			guard !hasAncestorRole(hit, role: "AXWebArea"), !textRoles.contains(hitRole) else { return false }
			guard supportsAction(hit, action: kAXPressAction as CFString) else { return false }
			guard AXUIElementPerformAction(hit, kAXPressAction as CFString) == .success else { return false }
			performed["grounding"] = "hit_test"
			performed["delivery"] = "ax"
			performed["hitRole"] = hitRole
			// 坐标点击没有 before/after 值可比,和后台物理事件一样无从自证 —— 如实说要调用方自己查。
			performed["verification"] = "caller_required"
			animateCursor(at: point, viaAx: true)
			return true
		}
		...
		func executeCoordinates(_ point: CGPoint) throws {
			...
			// 纯坐标点击先试后台 AX 按压。带 element 的路径不试:那边刚判过 AXPress 支不支持,
			// 走到这里就是已经失败过一次,再试一遍只是重复同一个失败。
			if element == nil, backgroundPressAtPoint(point) { return }
		...
		if let element, action == "press" || action == "click" {
			...
			if requiresPointerFocus && policy != "ax_only" {
				...
				if status == .success {
					...
					performed["delivery"] = "ax"
					if let cursorPoint { animateCursor(at: cursorPoint, viaAx: true) }
				} else {
					try executeCoordinates(coordinatePoint())
				}
			} else {
	...
	private func postMouseClick(at point: CGPoint, pid: Int32, button: CGMouseButton = .left, clickCount: Int = 1, delivery: String = "hid") throws {
		...
		// 守卫:只有指针仍停在我们投递的位置(= 这期间用户没自己动鼠标)才还原,绝不跟用户抢。
		let savedCursor: CGPoint? = delivery == "hid" ? CGEvent(source: nil)?.location : nil
		defer {
			// defer 里不能 return,只能靠嵌套 if
			if let savedCursor, let now = CGEvent(source: nil)?.location,
				abs(now.x - point.x) < 2, abs(now.y - point.y) < 2 {
				CGWarpMouseCursorPosition(savedCursor)
				// warp 之后必须重新关联,否则系统会短暂忽略真实鼠标移动(指针像卡住)。
