...
const helperBundleId = "com.forsion.tangu-computer-use";
...
const helperSourcePaths = ["foreground_activity.swift", "agent_cursor.swift", "agent_cursor_motion.swift", "agent_highlight.swift", "live_stream.swift", "bridge.swift"]
	.map((file) => path.join(rootDir, "native", "macos", file));
const packageJsonPath = path.join(rootDir, "package.json");
const releaseRepo = "forsion/tangu-computer-use";
// CN 必须 ≤64 字节(openssl x509 commonName 上限);品牌化后的全名会超,故缩写。
const localCodeSignCommonName = "Tangu CU Local Signing (com.forsion.tangu-computer-use)";
const localSigningLockPath = path.join(os.tmpdir(), `tangu-computer-use-local-signing-${typeof process.getuid === "function" ? process.getuid() : "user"}.lock`);
...
const defaultCodeSignIdentifier = "com.forsion.tangu-computer-use";
...
function prebuiltAppPathForArch(arch) {
	return path.join(rootDir, "prebuilt", "macos", arch, "tangu-computer-use.app");
}

const releaseAssetName = "tangu-computer-use.app.zip";
...
function moduleCachePath(arch) {
	return path.join(os.tmpdir(), `tangu-computer-use-swift-module-cache-${arch}`);
}
...
async function findExtractedHelperApp(extractDir) {
	const direct = path.join(extractDir, "tangu-computer-use.app");
	if (await exists(direct)) return direct;
	const entries = await fs.readdir(extractDir, { withFileTypes: true });
	for (const entry of entries) {
		if (!entry.isDirectory()) continue;
		const entryPath = path.join(extractDir, entry.name);
		if (entry.name === "tangu-computer-use.app") return entryPath;
...
async function downloadReleaseHelperApp() {
	...
	const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "tangu-computer-use-release-helper-"));
	try {
		...
		if (!appPath) throw new Error(`missing tangu-computer-use.app in ${releaseAssetName}`);
...
async function createLocalSigningIdentity() {
	const keychain = await loginKeychainPath();
	if (!keychain) return undefined;
	const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "tangu-computer-use-signing-"));
	const password = `tangu-computer-use-local-${process.pid}-${Date.now()}`;
	try {
		...
		console.warn(`[tangu-computer-use] could not create a valid local signing identity: ${error instanceof Error ? error.message : String(error)}`);
		return undefined;
	} finally {
		await fs.rm(tempDir, { force: true, recursive: true }).catch(() => {});
	}
...
/** 递归删掉 codesign 的临时残留(上一次签名被中断才会留下)。失败不致命,尽力而为。 */
async function removeSigningTempFiles(root) {
	const entries = await fs.readdir(root, { withFileTypes: true, recursive: true }).catch(() => []);
	for (const entry of entries) {
		if (!entry.isFile() || !entry.name.endsWith(".cstemp")) continue;
		await fs.rm(path.join(entry.parentPath ?? entry.path ?? root, entry.name), { force: true }).catch(() => {});
	}
}
...
async function signHelper(outputPath, identifier = defaultCodeSignIdentifier) {
	...
	// Tangu 补丁:先扫掉上一次被中断的 codesign 留下的 *.cstemp。
	// 不扫的话它会被当成普通资源**封进 CodeResources**,而 codesign 结束时又把它删了 ——
	// 于是签名从此恒定校验失败("a sealed resource is missing or invalid"),
	// 而 TCC 认的是签名,表现就是「权限开关怎么点都没用」。2026-07-28 真踩过,排查花了很久。
	await removeSigningTempFiles(outputPath);
	const commandArgs = ["--force", "--deep", "-i", identifier, "--timestamp=none", "--sign", identity, outputPath];
	await run("codesign", commandArgs);
	if (identity === "-") {
		console.warn("[tangu-computer-use] warning: signed helper ad-hoc; macOS may require permission review after native helper changes. Release installs should use a Developer ID-signed helper app.");
	} else if (identity === await findLocalSigningIdentity()) {
		console.log(`[tangu-computer-use] signed ${outputPath} with local identity '${localCodeSignCommonName}'. macOS may still require permission review after local rebuilds.`);
	}
	return identity;
}
...
async function setup() {
	...
	if (process.platform !== "darwin") {
		if (isPostinstall) {
			console.warn("[tangu-computer-use] skipping helper setup: platform is not macOS.");
			return;
		}
		throw new Error("tangu-computer-use helper is supported on macOS, Windows, and Linux. Use the matching --platform option.");
	}
	...
	if (prebuiltAppExists) {
		...
		console.log(
			installed
				? `[tangu-computer-use] installed pre-signed helper app (${arch}) at ${helperAppPath}`
				: `[tangu-computer-use] pre-signed helper app (${arch}) already current at ${helperAppPath}`,
		);
		return;
	}
	...
	if (prebuiltExists) {
		...
		console.log(
			installed
				? `[tangu-computer-use] installed helper app (${arch}) at ${helperAppPath}`
				: `[tangu-computer-use] helper app (${arch}) already current at ${helperAppPath}`,
		);
		return;
	}
	...
	try {
		...
		try {
			...
			console.log(
				installed
					? `[tangu-computer-use] installed signed helper app from GitHub Release ${releaseHelper.tag} (${releaseHelper.assetName}) at ${helperAppPath}`
					: `[tangu-computer-use] signed helper app from GitHub Release ${releaseHelper.tag} (${releaseHelper.assetName}) already current at ${helperAppPath}`,
			);
		} finally {
		...
		console.warn(`[tangu-computer-use] signed helper release download unavailable: ${error instanceof Error ? error.message : String(error)}`);
	}

	if (allowBuildFallback) {
		const tempPath = path.join(os.tmpdir(), `tangu-computer-use-bridge-${process.pid}-${Date.now()}`);
		try {
			console.log("[tangu-computer-use] prebuilt helper missing; attempting source build with xcrun swiftc...");
			await buildHelper(arch, tempPath);
			const installed = await installHelperApp(tempPath);
			console.log(
				installed
					? `[tangu-computer-use] built helper app at ${helperAppPath}`
					: `[tangu-computer-use] built helper app; installed app already current at ${helperAppPath}`,
			);
		} finally {
			await fs.rm(tempPath, { force: true }).catch(() => {});
		}
...
if (isMain) setup().catch((error) => {
	if (isPostinstall) {
		console.warn(`[tangu-computer-use] postinstall helper setup skipped: ${error instanceof Error ? error.message : String(error)}`);
		process.exit(0);
	}
